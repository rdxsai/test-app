    ...
    def update_question_and_answers(self, question_id: str, data: QuestionUpdate) -> bool:
        ...
        try:
            with self.get_connection(use_row_factory=False) as conn:
                ...
                # Only replace objectives when the client explicitly sends them.
                # When objective_ids is None (auto-save), leave associations untouched.
            ...
            with self.get_connection(use_row_factory=False) as conn:
                ...
                for ans in question_data['answers']:
                    ...
                    cursor.execute(
                        ...
                        VALUES (%s, %s, %s, %s, %s, %s)
                        """,
                        (new_a_id, new_q_id, ans['text'], ans['is_correct'], "", False)
                    )
