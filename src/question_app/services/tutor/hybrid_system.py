...
from .workers.content import (
    ...
    RetrievalWorker,
    TeachingContentPipelineOrchestrator,
...
class HybridCrewAISocraticSystem:
    ...
    def __init__(
        ...
        self._retrieval_bundle_builder = RetrievalBundleBuilder(
            wcag_mcp=self.wcag_mcp,
            retrieval_worker=self._retrieval_worker,
        )
        self._teaching_content_renderer = TeachingContentRenderer(
            bundle_builder=self._retrieval_bundle_builder,
        )
        self._teaching_content_pipeline = TeachingContentPipelineOrchestrator(
            plan_worker=self._teaching_plan_worker,
            concept_worker=self._concept_extraction_worker,
            retrieval_worker=self._retrieval_worker,
            bundle_builder=self._retrieval_bundle_builder,
    ...
    _AVAILABLE_TOOLS_TEXT = RetrievalWorker.AVAILABLE_TOOLS_TEXT

    def _assess_retrieval_coverage(
        self, objective_text: str, teaching_plan: Any, results: List[Dict[str, Any]],
    ) -> Dict[str, Any]:
        return self._retrieval_worker.assess_coverage(
    ...
    def _build_retrieval_feedback_message(
        ...
        return RetrievalWorker.build_feedback_message(
            coverage, remaining_calls, budget_chars
        )
    ...
    def _annotate_retrieval_results(
        ...
        return RetrievalWorker.annotate_results(planned_calls, results)
    ...
    def _extract_agentic_planned_calls(
        ...
        return self._retrieval_worker.extract_agentic_planned_calls(
    ...
    def _build_tool_description_lookup(tool_definitions: List[Dict[str, Any]]) -> Dict[str, str]:
        return RetrievalWorker.build_tool_description_lookup(tool_definitions)

    @staticmethod
    def _summarise_tool_result(result: Dict[str, Any]) -> str:
        return RetrievalWorker.summarise_tool_result(result)

    async def _run_agentic_retrieval(
        self, objective_text: str, teaching_plan: Any, ws_send,
    ) -> List[Dict[str, Any]]:
        artifact = await self._retrieval_worker.run_agentic_retrieval(
            objective_text=objective_text,
            teaching_plan=teaching_plan,
            ws_send=ws_send,
    ...
    async def _build_retrieval_bundle(
        ...
        return await self._retrieval_bundle_builder.build_bundle(
            results,
            objective_text=objective_text,
            teaching_plan=teaching_plan,
        )
    ...
    def _render_retrieval_bundle(
        ...
        return self._teaching_content_renderer.render(bundle, for_display=for_display)
